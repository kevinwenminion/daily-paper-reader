...
title_zh: 基于深度学习的多尺度问题LOD方法的代理建模
...
score: 7.0
evidence: 用于材料科学多尺度问题的深度学习代理模型
tldr: 多尺度问题因需精细离散化而计算昂贵，现有神经网络算子对强异质性系数精度不足。本文提出LOD-MSNO，融合局部正交分解（LOD）作为多尺度先验，通过数据驱动学习克服LOD计算瓶颈。理论误差估计表明，该方法在粗糙高对比系数椭圆PDE上显著提升精度，同时保持神经算子的计算效率。
source: arxiv
...
motivation: 传统多尺度数值方法（如LOD）计算成本高，而神经算子模型在强异质系数下精度不足，需结合二者优势。
method: 提出LOD-MSNO，以LOD问题自适应基函数为强多尺度先验，用神经算子学习映射系数以降低计算代价。
result: 理论误差估计成立，数值实验在粗糙高对比输入上精度超越现有神经算子，计算效率接近纯数据驱动模型。
conclusion: 混合数值先验与深度学习的框架有效平衡了多尺度问题的精度与效率。
---

## 摘要
多尺度问题使用传统数值方法处理非常困难，因为精确解析细尺度特征通常需要过于精细的离散化。这一挑战在材料科学、流体动力学、气候系统、化学过程和复杂网络等应用中尤为突出。最近的神经算子模型提供了一种有前景的数据驱动替代方案，但在存在强非均匀或振荡系数时，常常难以达到足够精度。本文中，我们关注具有粗糙和高对比度输入的椭圆偏微分方程的求解。局部正交分解（LOD）方法是一种成熟的处理此类问题的数值方法，但计算成本高昂。我们研究了流行的神经算子架构在这些具有挑战性的多尺度问题上的性能，并识别了它们在解析细尺度结构能力上的关键局限性。为克服这些挑战，我们引入了LOD-MSNO（LOD-多尺度神经算子），一种混合方法，它利用LOD方法作为强大的多尺度先验，基于其将解表示为问题自适应基函数的线性组合，并通过数据驱动的算子学习解决其主要计算瓶颈。我们进一步为所提出的系数学习框架提供了理论误差估计。最后，我们展示了所提方法在处理挑战性多尺度输入时，在精度上优于当前神经算子基线的潜力，同时主要保留了神经算子模型的计算效率。
